...
st.sidebar.title("User Login (Demo)")
...
if role == "Operator (Admin)":
    ...
    with col1:
        ...
        st.dataframe(df)
        st.subheader("Live Traffic Feed (sample)")
        st.json(traffic_feed)
        st.subheader("Live Weather Feed (sample)")
    ...
    with col2:
        ...
        if "current_plan" in st.session_state:
            ...
            st.json({
                ...
                "estimated_segment_minutes": p["estimated_segment_minutes"],
                "etas": p["etas"]
            })
            # Show map
    ...
    st.markdown("---")
    st.subheader("Monitor & Auto-Reroute (Demo)")
